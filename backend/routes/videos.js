...
// Upload Video (Partner only)
router.post('/upload', auth, verifyRole(['partner']), upload.single('video'), async (req, res) => {
    try {
        if (!req.file) {
...
// Get All Videos (User only)
router.get('/all', auth, verifyRole(['user']), async (req, res) => {
    try {
...
// Get Videos by Partner (Public/User/Partner)
...
// Like/Unlike Video (User only)
router.put('/like/:id', auth, verifyRole(['user']), async (req, res) => {
    try {
...
router.post('/comment/:id', auth, verifyRole(['user']), async (req, res) => {
    try {
        const { text } = req.body;
...
router.delete('/comment/:id', auth, verifyRole(['user']), async (req, res) => {
    try {
...
// Save/Unsave Video (User only)
router.post('/save/:id', auth, verifyRole(['user']), async (req, res) => {
    try {
        const videoId = req.params.id;
        const userId = req.user.id;
...
// Get Saved Videos (User only)
router.get('/saved', auth, verifyRole(['user']), async (req, res) => {
    try {
...
// ImageKit Auth Endpoint for Frontend Uploads
